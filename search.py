...
class Trainer(object):
    def __init__(self, args):
        ...
        print('Number of Feature Net parameters: {}'.format(
            sum([p.data.nelement() for p in self.model.module.feature.parameters()])))
        print('Number of Matching Net parameters: {}'.format(
            sum([p.data.nelement() for p in self.model.module.matching.parameters()])))
    ...
    def training(self, epoch):
        ...
        for i, batch in enumerate(tbar):
            input1, input2, target = Variable(batch[0], requires_grad=True), Variable(batch[1], requires_grad=True), (
            batch[2])
            if self.args.cuda:
                input1 = input1.cuda()
                input2 = input2.cuda()
            ...
            if valid > 0:
                ...
                if epoch >= self.args.alpha_epoch:
                    ...
                    input1_search, input2_search, target_search = Variable(search[0], requires_grad=True), Variable(
                        search[1], requires_grad=True), (search[2])
                    if self.args.cuda:
                    ...
                    arch_loss = F.smooth_l1_loss(output_search[mask_search], target_search[mask_search],
                                                 reduction='mean')
        ...
        for i, batch in enumerate(tbar):
            ...
            if valid > 0:
                with torch.no_grad():
                    ...
                    print("===> Test({}/{}): Error(EPE): ({:.4f} {:.4f})".format(i, len(self.val_loader), error.item(),
                                                                                 three_px_acc))
